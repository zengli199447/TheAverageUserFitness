package com.example.administrator.sportsfitness.wxapi;
    ...
    public void onResp(BaseResp resp) {
        if (resp.getType() == ConstantsAPI.COMMAND_PAY_BY_WX) {
            if (resp.errCode == 0) {
                ...
                LogUtil.e(TAG, "支付成功");
                RxBus.getDefault().post(new CommonEvent(DataClass.PAY_RETURN_STATUS, 0));
                ...
                RxBus.getDefault().post(new CommonEvent(DataClass.PAY_RETURN_STATUS, 1));
            }
        } else {
            if (resp.errCode == -2) {
                Toast.makeText(this, "支付已取消", Toast.LENGTH_SHORT).show();
